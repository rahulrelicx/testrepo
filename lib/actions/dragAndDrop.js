...
const dragAndDrop = async (source, destination, options) => {
  ...
  await doActionAwaitingNavigation(options, async () => {
    ...
    sourceDraggable
      ? await dragAndDropHTML5Element(sourcePosition, destPosition, sourceElem.get())
      : await dispatchDragAndDropMouseEvent(options, sourcePosition, destPosition);
  });
...
const dispatchDragAndDropMouseEvent = async (options, sourcePosition, destPosition) => {
...
const dragAndDropHTML5Element = async (sourcePosition, destPosition, sourceElementObjectId) => {
  const args = { sourcePosition, destPosition };
  async function simulateDragAndDrop(args) {
